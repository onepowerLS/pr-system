...
export function generatePendingApprovalEmail(context: NotificationContext): EmailContent {
  ...
  const subject = isUrgent ? `URGENT: PR #${prNumber} Pending Your Approval` : `PR #${prNumber} Pending Your Approval`;
  ...
  const html = `
    <div style="${styles.container}">
      ...
      <h2 style="${styles.header}">Purchase Request #${prNumber} Requires Your Approval</h2>
      ...
      ${notes ? `<div style="${styles.notes}"><strong>Notes:</strong> ${notes}</div>` : ''}
  ...
  const text = `${isUrgent ? 'URGENT: ' : ''}PR #${prNumber} Pending Your Approval

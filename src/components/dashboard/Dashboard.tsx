...
export const Dashboard = () => {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        // Load user's PRs
        const userPRsData = await prService.getUserPRs(user.id);
        ...
        if (user.role === UserRole.APPROVER || user.role === UserRole.ADMIN) {
          const pendingApprovalsData = await prService.getPendingApprovals(user.id);
          ...
          <Paper sx={{ p: 2 }}>
            ...
            <Grid container spacing={2}>
              {Object.values(PRStatus).map((status) => {
                const count = userPRs.filter((pr) => pr.status === status).length;
                return (
